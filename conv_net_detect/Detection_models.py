...
from CONFIG import DROPOUT, ANCHOR_BOXES
...
class DetectionModel(torch.nn.Module):
	def __init__(self, number_classes, grid_size_width, grid_size_height, chosen_model):
		...
		if chosen_model == 0:
			...
			self.convolutional_layers = self._create_convolutional_layers_ResNet50()
			self.fully_connected_layers = self._create_fully_connected_layers(grid_size_width=self.grid_size_width, grid_size_height=self.grid_size_height, number_classes=self.number_classes,
	...
	def forward(self, x):
		return self.fully_connected_layers(self.convolutional_layers(x)).reshape((x.shape[0], self.grid_size_height, self.grid_size_width, self.number_classes + 3 + 2 * len(ANCHOR_BOXES)))
	...
	@staticmethod
	def _create_convolutional_layers_ResNet50():
	...
	def _create_fully_connected_layers(grid_size_width, grid_size_height, number_classes, num_input_params):
		return torch.nn.Sequential(torch.nn.Flatten(), torch.nn.Linear(num_input_params, 1024), torch.nn.Dropout(DROPOUT),
		                           torch.nn.LeakyReLU(0.1), torch.nn.Linear(1024, grid_size_width * grid_size_height * (number_classes + 3 + 2 * len(ANCHOR_BOXES))))
